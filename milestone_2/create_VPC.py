...
subnetLabel = tk.Label(root,text = 'Subnet')
subnetLabel.pack()
subnetEntry = tk.Entry(root, textvariable = subnet)
subnetEntry.pack()
...
for i,j in enumerate(l):
    for k in range(1,len(l)):
        ...
        #print(nsl[i])
        res = subprocess.check_output(["sudo","ip","netns","exec",nsl[i],"ip","tunnel","add",gretun,"mode","gre","local",s_gre_ip.split('/')[0],"remote",d_gre_ip.split('/')[0]])
        res = subprocess.check_output(["sudo","ip","netns","exec",nsl[i],"ip","link","set","dev",gretun,"up"])
...
for i,j in enumerate(l):
    for k in range(1,len(l)):
        ...
        res = subprocess.check_output(["sudo","ip","netns","exec",nsl[i],"ip","route","add",gre_pair_ip,"dev",gretun])
