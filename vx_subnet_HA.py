...
titleLabel = tk.Label(root, text = 'Create Subnet with High Availability')
titleLabel.pack()
...
#op = raw_input("Enter the option that you want to create your subnet cloud: ")
...
for i,j in enumerate(s):
    ...
    if1 = j+"-s"+s_ip+"_if1"
    if2 = j+"-s"+s_ip+"_if2"
    bk1 = j+"-s"+s_ip+"_bk1" #backup if1
    bk2 = j+"-s"+s_ip+"_bk2" #backup if2
    br = j+"-s"+s_ip+"_br"
    br_bk = j+"-s"+s_ip+"_bk"
    ipa = subnet.split("/")[0]
    i_off = ipa.split(".")
    ...
    res = subprocess.check_output(["sudo","ip","link","add",bk1,"type","veth","peer","name",bk2]) #backup
    ...
    #ip addressing for backup to be done when failover
    ...
    subprocess.check_output(["sudo","docker","exec","--privileged",j,"brctl","addbr",br_bk]) #bk
   
    # veth pair to bridge

    subprocess.check_output(["sudo","docker","exec","--privileged",j,"brctl","addif",br,if2])
    subprocess.check_output(["sudo","docker","exec","--privileged",j,"brctl","addif",br_bk,bk2]) #bk
    # ip addr to bridge, turn up
    subprocess.check_output(["sudo","docker","exec","--privileged",j,"ip","addr","add",brip+"/24","dev",br])
    #ip addressing for backup br to be done when failover
    subprocess.check_output(["sudo","docker","exec","--privileged",j,"ip","link","set", "dev", br,"up"])
    subprocess.check_output(["sudo","docker","exec","--privileged",j,"ip","link","set", "dev", br_bk,"up"]) #bk
    ...
    #default route to be changed on failover
...
for i,j in enumerate(s):
	...
	# can't attach a vxlan device to two bridges simultaneously
